...
##  Author: Ioannis Moustakas (i.moustakas@uva.nl), based on the scrip of Iris Kolder   ##
##########################################################################
...
HotSpots <- function(variantsPos, windowSize, variants){
  ...
  #hotspotIndexes <- match(hotspotPosList, variantsPos)  ###### This doesnt work when spliting the SNPs
  hotspotIndexes <- which(variantsPos %in% hotspotPosList)
...
annotateSNPTable <- function (path, annotFile, tableOfSNPs) {
  ...
  extractGeneAttribute <- function(attribute) {
    if (length(attribute)==0){
      ...
      nameField <- strsplit(attribute, ";", fixed=T)[[1]]
  ...
  for (contigID in contigsInAnnot) {
    ...
    repeat {
      ...
      if (element ==  "gene") {  
        ...
        elementPosition <- as.numeric(annotationEnhanced$start) > previousElementStopCoordinates & as.numeric(annotationEnhanced$start) < elementStartCoordinates
  ...
  for (chromID in chromIDsTableOfSNPs) {
    ...
    geneAttributes <- sapply(SNPPos, function(x) {
      ...
      if (length(attributeString) == 0) {attributeString <- fractionTable[fractionTable$feature=="tRNA", ]$attribute}
...
ruleTwovariantIDs <- unlist(UBTwovariantIDs)
...
CompileExperiment <- function(sampleIDs, tableOfSNPs, tableOfSNPsSmallVariants, experimentName,
  ...
  # Append, averageRowDepth,  averageRowQual, Elements, GeneName, geneFunction and Hotspot consensus  columns
  ...
  compiledExperimentTable$GeneFunction <- experimentTableUniqueVar$GeneFunction
...
CompileExperiment(sampleIDsForCAZ256, tableOfSNPs, tableOfSNPsSmallVariants, "CAZ256_BaseClReferenceCons",
                  germlineVariantIDs, UBOneVariantIDs, ruleTwovariantIDs)
...
CompileExperiment(sampleIDsForMPN256, tableOfSNPs, tableOfSNPsSmallVariants, "MPN256_BaseClReferenceCons",
                  germlineVariantIDs, UBOneVariantIDs, ruleTwovariantIDs)
...
CompileExperiment(sampleIDsForPT512, tableOfSNPs, tableOfSNPsSmallVariants, "PT512_BaseClReferenceCons",
                  germlineVariantIDs, UBOneVariantIDs, ruleTwovariantIDs)
...
CompileExperiment(sampleIDsForTBM32, tableOfSNPs, tableOfSNPsSmallVariants, "TBM32_BaseClReferenceCons",
                  germlineVariantIDs, UBOneVariantIDs, ruleTwovariantIDs)
...
CompileExperiment(sampleIDsForCIP128, tableOfSNPs, tableOfSNPsSmallVariants, "CIP128_BaseClReferenceCons",
                  germlineVariantIDs, UBOneVariantIDs, ruleTwovariantIDs)
...
CompileExperiment(sampleIDsForTBM1024, tableOfSNPs, tableOfSNPsSmallVariants, "TBM1024_BaseClReferenceCons",
                  germlineVariantIDs, UBOneVariantIDs, ruleTwovariantIDs)
...
CompileExperiment(sampleIDsForCIP256, tableOfSNPs, tableOfSNPsSmallVariants, "CIP256_BaseClReferenceCons",
                  germlineVariantIDs, UBOneVariantIDs, ruleTwovariantIDs)
