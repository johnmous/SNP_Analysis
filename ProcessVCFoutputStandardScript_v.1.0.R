...
ReadVCF <- function(listOfFiles) {
  ...
  for (i in 1:length(listOfFiles)) {
    ...
    if (!inherits(possibleError, "error")){
      # read file and store
      Sample <- read.table(paste(path, listOfFiles[i], sep = ""), sep="\t", head=F, comment.char = "#")
...
write.table(tableOfSNPs, paste(path, "CompliledExperimentTable.txt"), sep="\t", row.names=FALSE) 
