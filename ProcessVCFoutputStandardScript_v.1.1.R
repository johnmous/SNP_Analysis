##########################################################################
##  Title: Process standard VCF files and compile a table with the variants from all samples, annotated
##  Author: Ioannis Moustakas (i.moustakas@uva.nl)	[Based on Iris Kolder script]				##
##	last edited: 22-04-2015						##
##########################################################################
...
path="/zfs/datastore0/group_root/MAD-RBAB/02_Collaborators/MAD1207-Puratos/MAD1207-P001-Bacillus_Subtilis/MAD1207-P001-E002_2015_gDNA_analysis_svleeuw1/Scratch/VCFCompiled/"
# Name for annotation file
annotFile = "BSubltilis168_NCBI.gff"
referenceFile = "bsub_168.fasta"

# Load reference sequence
reference <- read.fasta(paste(path, referenceFile, sep=""), seqtype= "DNA")
...
#################### $$$$$$$$$$$$$$$$$$$ ##################### 
# A function needed to manipulate the annotation table
...
annotateSNPTable <- function (path, annotFile, tableOfSNPs) {
  ...
  annotation <- read.table(paste0(path, annotFile), sep="\t", head=TRUE, quote="", stringsAsFactors=FALSE)
  ...
  extractGeneName <- function(attribute) {
                      ...
                      nameField <- strsplit(attribute, ";", fixed=T)[[1]][2]
  ...
  for (contigID in contigsInAnnot) {
    ...
    repeat {
      ...
      if ( gapBetweenElements > 1 ) { #& element ==  "gene") {  
        ...
        newRow <- c( contigID, annotationSlice$RefSeq[1], "NonCoding", previousElementStopCoordinates+1, 
                     elementStartCoordinates-1, "score", strand, "Frame", newAttribute ) 
...
ReadVCF <- function(listOfFiles) {
  ...
  for (i in 1:length(listOfFiles)) {
    ...
    if (!inherits(possibleError, "error")){
      # read file and store
      Sample <- read.table(paste(path, listOfFiles[i], sep = ""), sep="\t", head=F, comment.char = "#", stringsAsFactors=F)
      ...
      ### This is made redundant, as it is calculated again further down
      ### In any case, it didnt work properly for the POS with Multiple Variants
...
#################### $$$$$$$$$$$$$$$$$$$$ #####################
### Remove the SNPs that are present across all samples and their maxFreq-minFreq is smaller than 20%
### 
RemoveGermline <- function(tableOfSNPs){
  variantIDs <- as.factor(tableOfSNPs$VariantID)
  numberOfSamlpes <- length(unique(tableOfSNPs$ID))
  variantIDsTable <- table(variantIDs)
  ## Boolean variable, True when the same variant is seen across all samples
  germlineAcrossAllSamplesVarID <- row.names(variantIDsTable[variantIDsTable==numberOfSamlpes])
  isGermline = vector()
  for (varID in germlineAcrossAllSamplesVarID) {
    sliceTableGermaline <- tableOfSNPs[tableOfSNPs$VariantID==varID, ]
    sliceFrequences <- as.numeric(sliceTableGermaline$SNPFreq)
    diff = max(sliceFrequences) - min(sliceFrequences)
    ######## Only if the max-min diff is bellow 0.2, then SNP is germline ##########
    if (diff<0.2){
      isGermline <- c(isGermline, varID) 
    }
  }
  
  germline <- tableOfSNPs$VariantID %in% isGermline
  
  #### Remove the lines that are the same across all samples
  tableOfSNPs <- tableOfSNPs[ !germline, ]
  return(tableOfSNPs)
}

#################### $$$$$$$$$$$$$$$$$$$$ #####################
# get the names of all vcf files in the directory and run the function that reads them 
...
### Remove Germlines from the table
tableOfSNPs <- RemoveGermline(tableOfSNPs)
...
# Drop the germLine column

tableOfSNPs <- tableOfSNPs[ ,!(names(tableOfSNPs) %in% c('Germline', 'VariantID'))]

# Flip the last two columns of the data frame
#columnNames <- colnames(tableOfSNPs)
#columnsOrdered <- columnNames[c(1:13, 15, 14)]
#tableOfSNPs <- tableOfSNPs[columnsOrdered]                          
            
write.table(tableOfSNPs, paste(path, "CompliledExperimentTableGermLine.txt"), sep="\t", row.names=FALSE) 
...
# ############ tableOfSNPsSmallVariants <- tableOfSNPsSmallVariants[ ,c("CHROM", "POS", "TYPE", "ID")]
# 
...
# # 
# # ###################### $$$$$$$$$$$$$$$$$$$$$$ #######################
# # # Calculate Rule 1 as a vector equal to the length of the first sample (WT)
# # # SNPs with freq > 0.95 ==> Germline,
# # # 0.05 < freq < 0.95 ==> Unreadable Base 1
# # # else leave as is
# # # create an extra colunt concisting of the Contig+POS+Type of SNP. That string is a unique identifier for every variant
# # # If there are two different var types in the same pos, they are both included
# # variantID <- paste(tableOfSNPs$CHROM, tableOfSNPs$POS, tableOfSNPs$TYPE, sep="_" )
# # tableOfSNPs$VariantID <- variantID
# # allSampleIDs <- sort(unique(tableOfSNPs$ID))
# # WTZeroVariants <- tableOfSNPs[ tableOfSNPs$ID == allSampleIDs[1], ]
# # WTZeroVArIDs <- WTZeroVariants$VariantID
# # SNPFreq <- WTZeroVariants$SNPFreq
# # sumSNPFreq <- sapply(SNPFreq, function(string){
# #   splitString <- as.numeric(unlist(strsplit(string, split=",", fixed=T)))
# #   sum(splitString)
# # })
# # WTZeroVariants$sumSNPFreq <- sumSNPFreq
# # ruleOne <- lapply(sumSNPFreq, function(freq){
# #   if (freq > 0.95) return("Germline")
# #   else if (freq > 0.05) return("UB1")
# #   else return("asIs")  
# # })
# # ruleOne <- unlist(ruleOne)
# # germlineVariantIDs <- WTZeroVArIDs[ruleOne=="Germline"]
# # UBOneVariantIDs <- WTZeroVArIDs[ruleOne=="UB1"]
# # 
# # #################### $$$$$$$$$$$$$$$$$$$$$ ######################
# # # Calculate Rule 2
# # # First compile a data.frame with all the variants "collapsed" across samples
# # collapsedVariantsTable <- data.frame()
# # uniqueVars <- unique(variantID)
# # collapsedVariantsTable <- data.frame(matrix(NA, nrow = length(uniqueVars), ncol = length(allSampleIDs) ,
# #                                             dimnames = list(1:length(uniqueVars), names(allSampleIDs))))
# # i = 1
# # for (sample in allSampleIDs){
# #   sliceOfTableForComparisson <- tableOfSNPs[ tableOfSNPs$ID == sample, ]
# #   SNPFreq <- sapply(uniqueVars, function(variant){                        
# #     SNPFreq <- sliceOfTableForComparisson[ sliceOfTableForComparisson$VariantID  == variant, ]$SNPFreq
# #     if (length(SNPFreq) != 0) {
# #       splitString <- as.numeric(unlist(strsplit(SNPFreq, split=",", fixed=T)))
# #       sumSNPFreq <- sum(splitString)
# #       return(sumSNPFreq)
# #     } else {
# #       return(0)
# #     }
# #   })
# #   collapsedVariantsTable[ ,i] <- SNPFreq
# #   i = i+1
# # }
# # 
# # colnames(collapsedVariantsTable) <- allSampleIDs
# # collapsedVariantsTable$VariantID <- uniqueVars
# # UBTwovariantIDs <- apply(collapsedVariantsTable, 1, function(row){
# #   frequencies <- as.numeric(head(row, length(row)-1))
# #   variantID <- tail(row, 1)
# #   for (i in 1:10){
# #     mean <- mean(frequencies)
# #     absFrequencies <- abs(frequencies - mean)
# #     maxDistance <- max(absFrequencies)
...
# alternativeBases <- tableOfSNPs$ALT
# alternativeBasesSplit <-  strsplit(alternativeBases, split=',', fixed=T)
# # Check which lines have multiple alternatives reported and save it in a boolean vector
# multipleAlternativesBoolean <- sapply(alternativeBasesSplit, function(x){
#                                 length(unlist(x))>1
#                                 })
# tableOfSNPs$MultipleAlt <- multipleAlternativesBoolean